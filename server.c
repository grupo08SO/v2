...
MYSQL_ROW *row;
//-----------------------------------------------------
//****************Funcion:anadir usuario a la lista****************
//int add_to_list(TUsuarios* l, int socket, char nombre[30])
//{
//	//Check if there is space in the list
//	if ((l->num) < 100)
//	{
//		strcpy(l->list[(l->num)].nombre, nombre);
//		l->list[(l->num)].socket = socket;
//		l->num = (l->num) + 1;
//		return 0;
//	}
//	else
...
////****************Funcion:conseguir posicion del usuario en la lista de conectados****************
//int dame_posicion(TUsuarios* l, char nombre[30]) {
//	int encontrado = 0;
//	int	i = 0;
//	printf("%s\n", l->list[0].nombre);
//	while ((encontrado == 0) && (i < (l->num)))
//	{
//		if (strcmp(l->list[i].nombre, nombre) == 0)
//		{
//			encontrado = 1;
//		}
//		else
...
//	}
//}
//****************Funcion:eliminar usuario de la lista****************
//int remove_from_list(TUsuarios* l, char nombre[30])
//{
//	int	i = 0;
//	int p = dame_position(l, nombre);
//	printf("%d\n", p);
//	if (p != -1) {
//		while (p < (l->num))
//		{
//			strcpy(l->list[p].nombre, l->list[p + 1].nombre);
//			l->list[p].socket = l->list[p + 1].socket;
//			l->num = l->num - 1;
//			p++;
//
//			return 0;
...
//
//}
//****************Funcion:obtener lista de los usuarios conectados****************
void get_connected(TUsuarios* l, char reply[512])
{
	int i = 0;
	char p[100];
	strcpy(reply, "3/");
...
//****************Funcion:obtener socket de un usuario****************
//int get_socket(TUsuarios* l, char nombre[30])
//{
//	int i = 0;
//	int stop = 0;
//	while ((i < (l->num)) && (stop == 0))
//	{
//		if (strcmp((l->list[i].nombre), nombre) == 0)
//			stop = 1;
//		else
...
{ 
	...
	if (codigo == 1){
		if(LogIN(input, output))
			...
			char inform[20];
			get_connected(&l, inform);
			int j;
			for (j = 0; j < (l.num); j++)
			{
				write(l.list[j].socket, inform, strlen(notification));
			}
			printf("%s\n", inform);
			else
